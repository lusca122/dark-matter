...
for i,v in next, character.Model:GetChildren() do
	v.Transparency = 1
end
...
function attach(Part0,Part1,Position,Angle)
    ...
    Part0.AccessoryWeld:Remove()
    return {AlignPos,AlignOri,AttachmentA,AttachmentB}
end
--local Char = workspace.non
--[[
local Hats = {
 Hat1 = Char:FindFirstChild("MeshPartAccessory"),
}

if Char:FindFirstChild("ShadowBladeMasterAccessory") then
	Hats.Hat1 = Char:FindFirstChild("ShadowBladeMasterAccessory")
end
--]]
if _G.permadeath == true then
if char:FindFirstChild("MeshPartAccessory") then
    local athp = Instance.new("Attachment",char.Model.Part)
    local atho = Instance.new("Attachment",char.Model.Part)
    if char["MeshPartAccessory"] then
        char["MeshPartAccessory"].Handle.AccessoryWeld:Remove()
        char["MeshPartAccessory"].Handle:FindFirstChildOfClass("AlignPosition").Attachment1 = athp
        char["MeshPartAccessory"].Handle:FindFirstChildOfClass("AlignOrientation").Attachment1 = atho
    end
    athp.Position = Vector3.new(-2.65, 0.05, 0)
    atho.Orientation = Vector3.new(-0, -0, -48)
end
else
attach(char:FindFirstChild("MeshPartAccessory").Handle,char.Model.Part,Vector3.new(-0.1, -0.3, 0),Vector3.new(0, 90, 65))
end
